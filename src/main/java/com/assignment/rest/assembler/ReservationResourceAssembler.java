...
public class ReservationResourceAssembler extends ResourceAssemblerSupport<Reservation, Resource> {
    ...
    public Resource<Reservation> toResource(Reservation reservation) {
        Link self = entityLinks.linkToSingleResource(Reservation.class, reservation.getId()).withSelfRel();
        Link room = entityLinks.linkToSingleResource(Room.class, reservation.getRoom().getId());
        Link employee = entityLinks.linkToSingleResource(Employee.class, reservation.getEmployee().getId());
